...
int main(void) {
    ...
    //   dpo canvas = dpo(make_pair(0, 0), make_pair(0, 0));
    // HAL_UART_Receive_IT(&huart1, (uint8_t *)rxBuffer, 1);
    TP_Init();
    LCD_Clear(WHITE);
    // Init Display Object
    POINT_COLOR = (RED);
    LCD_ShowString(90, 50, 200, 16, 16, (uint8_t *)"Mini STM32");
    POINT_COLOR = BLUE;
    LCD_ShowString(90, 70, 200, 16, 16, (uint8_t *)"TOUCH TEST");
    POINT_COLOR = GREEN;
    LCD_ShowString(90, 90, 200, 16, 16, (uint8_t *)"ATOM@ALIENTEK");
    POINT_COLOR = YELLOW;
    LCD_ShowString(90, 110, 200, 16, 16, (uint8_t *)"2019/11/15");
    POINT_COLOR = BLACK;
    ctp_test();
    ...
    while (1) {
        ...
        // printf("inited\n");
        // HAL_Delay(100);
        // dpo canvas = dpo("good", {0, 0}, {0, 0});
        // char str[30];
        // sprintf(str, "%d", canvas.get_id());
        // LCD_ShowString(30, 70, 200, 16, 16, (uint8_t *)str);
        /* USER CODE BEGIN 3 */
        LCD_ShowString(120, 160, 50, 16, 16, Rx_Data);
    }
    /* USER CODE END 3 */
}
