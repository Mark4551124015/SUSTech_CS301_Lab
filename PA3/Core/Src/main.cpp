...
int main(void) {
    ...
    main_menu scene_1 = main_menu("Main Menu", {0, 0}, window_view.shape);
    window_view.add_son(&scene_1);

    sld scene_2 = sld("Slide Validation", {0, 0}, window_view.shape);
    window_view.add_son(&scene_2);
    scene_2.setVisbility(false);

    tch_scene scene_3 = tch_scene("Touch Scene", {0, 0}, window_view.shape);
    window_view.add_son(&scene_3);
    scene_3.setVisbility(false);

    finish_scene scene_4 = finish_scene("Finish", {0, 0}, window_view.shape);
    window_view.add_son(&scene_4);
    scene_4.setVisbility(false);
    ...
    while (1) {
        ...
        if (EVENT[START_GAME]) {
            printf("[EVENT] Start Game\n");
            scene_1.setVisbility(false);
            scene_2.reset();
            scene_2.setVisbility(true);
            EVENT[START_GAME] = 0;
            canvas.need_render = true;
        }
        if (EVENT[FINISH_1]) {
            printf("[EVENT] Finish 1\n");
            scene_2.setVisbility(false);
            scene_3.reset();
            scene_3.setVisbility(true);
            EVENT[FINISH_1] = 0;
            canvas.need_render = true;
        }
        if (EVENT[FINISH_2]) {
            printf("[EVENT] Finish 2\n");
            scene_3.setVisbility(false);
            scene_4.setVisbility(true);
            EVENT[FINISH_2] = 0;
            canvas.need_render = true;
        }
