...
// uint16_t screen[240][320];
// uint16_t old_screen[240][320];
...
// //     LCD_WR_DATA(LCD_BGR2RGB(POINT_COLOR));  // POINT_COLOR: Color of this
// //     point
// // }

// void LCD_DrawPoint(uint16_t x, uint16_t y) {
//     screen[x][y] = (POINT_COLOR);  // POINT_COLOR: Color of this point
// }

// // Draw a point with color(quicker)
// // x,y: coordinate
// void origin_LCD_Fast_DrawPoint(uint16_t x, uint16_t y, uint16_t color) {
//     LCD_WR_REG(lcddev.setxcmd);
//     LCD_WR_DATA(x >> 8);
//     LCD_WR_DATA(x & 0XFF);
//     LCD_WR_REG(lcddev.setycmd);
...
// void origin_LCD_Fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
//                      uint16_t color) {
//     uint16_t i, j;
//     uint16_t xlen = 0;
//     uint16_t temp;
...
//     for (i = sy; i <= ey; i++) {
//         for (j = 0; j < xlen; j++)
//             screen[j][i] = (LCD_BGR2RGB(color));  // 设置光标位置
//     }
// }

// // Fill the area with color
// //(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)
// void origin_LCD_Color_Fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
//                            uint16_t *color) {
...
//         LCD_WriteRAM_Prepare();     // 开始写入GRAM
//         for (j = 0; j < width; j++)
//             screen[j][i] = (LCD_BGR2RGB(color[i * width + j]));  // 写入数据
//     }
// }
...
//     for (h = p1_y; h < p2_y; h++) {
//         for (m = p1_x; m < p2_x; m++) {
//             uint32_t shift = h * row + m;
//             LCD_Fast_DrawPoint(x + m - p1_x, y + h - p1_y, *(data + shift));
//         }
//     }
//     LCD_DrawRectangle(x, y, x + p2_x - p1_x, y + p2_y - p1_y);
// }

// void LCD_update() {
//     for (int i = 0; i < 320; i++) {
//         for (int j = 0; j < 240; j++) {
//             if (screen[j][i] != old_screen[j][i]) {
//                 origin_LCD_Fast_DrawPoint(j, i, screen[j][i]);
//                 old_screen[j][i] = screen[j][i];
//             }
//         }
//     }
// }