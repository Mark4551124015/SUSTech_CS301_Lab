...
int main(void) {
    ...
    while (1) {
        ...
        leddev.tick();
        if (plain_text.isClicked()) {
            input_binary.setVisbility(default_keyboard.isVisible);
            default_keyboard.setVisbility(!default_keyboard.isVisible);
            secret_text.setVisbility(!default_keyboard.isVisible);
            secret_text.choosing = false;
        }
        if (secret_text.isClicked()) {
            input_binary.setVisbility(input_binary.isVisible);
            plain_text.choosing = false;
        }

        if (choosed == &secret_text) {
            if (left.isClicked()) {
                choosed->move_cursor(choosed->cursor - 1, choosed->my_axis);
            }
            if (right.isClicked()) {
                choosed->move_cursor(choosed->cursor + 1, choosed->my_axis);
            }
            if (zero.isClicked()) {
                choosed->add_on_cursor('0');
                leddev.append(BLINK_0);
            }
            if (one.isClicked()) {
                choosed->add_on_cursor('1');
                leddev.append(BLINK_1);
            }
            if (backspace.isClicked()) {
                choosed->delete_on_cursor();
            }
        }
        if (choosed == &plain_text) {
            char c = default_keyboard.typing();
            if (c != 0) {
                plain_text.add_on_cursor(c);
                encode_single_hfm(c, tmp);
            }
            if (default_keyboard.left->isClicked()) {
                choosed->move_cursor(choosed->cursor - 1, choosed->my_axis);
            }
            if (default_keyboard.right->isClicked()) {
                choosed->move_cursor(choosed->cursor + 1, choosed->my_axis);
            }
            if (default_keyboard.del->isClicked()) {
                choosed->delete_on_cursor();
            }
        }
        ...
        if (encrypt.isClicked()) {
            secret_text.clear();
            encode_hfm(plain_text.str, secret_text.str, secret_text.max_len);
            int len = strlen(secret_text.str);
            secret_text.len = len < secret_text.max_len ? len : secret_text.max_len;
            secret_text.cursor = 0;
            secret_text.need_render = true;
            printf("cursor: %d, len: %d\n", secret_text.cursor,
                   secret_text.len);
        }

        if (decrypt.isClicked()) {
            plain_text.clear();
            decode_hfm(secret_text.str, plain_text.str,plain_text.max_len);
            int len = strlen(plain_text.str);
            plain_text.len = len < plain_text.max_len ? len : plain_text.max_len;
            plain_text.len = strlen(plain_text.str);
            plain_text.cursor = 0;
            plain_text.need_render = true;
            printf("cursor: %d, len: %d\n", secret_text.cursor,
                   secret_text.len);
        }

        // State Machine
        if (default_keyboard.isVisible) {
            // offset of keyboard showing
            for (int i = 0; i < 2;i++) leddev.tick();
        } else {
            delay_ms(1);
        }
    }
